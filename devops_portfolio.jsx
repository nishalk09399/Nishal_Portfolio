  ...
  return (
    <div className="bg-gray-50 min-h-screen text-gray-900">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-5xl mx-auto px-6">
          {["Jenkins","Terraform","Ansible","Docker","Kubernetes","AWS","SonarQube","Grafana","GitLab"].map((skill, i) => (
            <Card key={i} className="shadow-md text-center">
              <CardContent className="p-4 font-semibold">{skill}</CardContent>
            </Card>
          ))}
        </div>
      </section>
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {["AWS Certified Solutions Architect - Associate","AWS Cloud Practitioner","Azure Fundamentals AZ-900"].map((cert, i) => (
            <Card key={i} className="shadow-md">
              <CardContent className="p-4 font-semibold text-center">{cert}</CardContent>
            </Card>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 py-16 text-center">
        ...
        <Button asChild>
          <a href="mailto:nishalk9399@gmail.com">Say Hello</a>
        </Button>
      </section>
    </div>
  );
